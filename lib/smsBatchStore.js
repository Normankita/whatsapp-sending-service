...
function getStatusForPhones(phones) {
  const { batches } = readBatches();
  ...
  for (const batch of batches) {
    for (const recipient of batch.recipients || []) {
      ...
      if (!phoneMap.has(phone)) {
        phoneMap.set(phone, { everSucceeded: false, attempts: [] });
      }
      const entry = phoneMap.get(phone);
      if (recipient.status === 'sent') entry.everSucceeded = true;
      entry.attempts.push({
        batchId: batch.id,
        batchCreatedAt: batch.createdAt,
        status: recipient.status,
  ...
  for (const phone of phones) {
    const entry = phoneMap.get(phone);
    if (!entry) {
      results[phone] = { status: 'never_attempted', reason: null, attemptCount: 0 };
      continue;
    }
    const sortedAttempts = [...entry.attempts].sort(
      (a, b) => new Date(b.batchCreatedAt) - new Date(a.batchCreatedAt)
    ...
    results[phone] = {
      status: entry.everSucceeded ? 'sent' : 'failed',
      reason: entry.everSucceeded ? null : (sortedAttempts[0]?.reason || null),
      attemptCount: sortedAttempts.length,
    };
  }
  return results;
...
function getAllKnownPhones() {
  const { batches } = readBatches();
  const phones = new Set();
  for (const batch of batches) {
    for (const recipient of batch.recipients || []) {
      if (recipient.phone) phones.add(recipient.phone);
    }
  }
  return [...phones];
}

module.exports = { createBatch, getAllBatches, getBatchById, updateBatchMessage, updateRecipientStatuses, getNeverDeliveredRecipients, getAllKnownPhones, getStatusForPhones };
